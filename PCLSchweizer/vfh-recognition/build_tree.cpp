...
main (int argc, char** argv)
{
  ...
  /*loadFeatureModels方法除了递归遍历一组目录和子目录，并且在它找到的所有.PCD文件中加载时，
  它什么都不做。在 loadFeatureModels，我们调用loadHist，它将尝试打开找到的每个PCD文件，
  读取其标题，并检查它是否包含VFH签名。与VFH签名一起，我们还将PCD文件名存储为一 vfh_model对。
  */
  ...
  // Convert data into FLANN format加载所有VFH功能后，我们将它们转换为FLANN格式
  ...
  // Save data to disk (list of models)由于我们很懒，而且我们想要使用这些数据（而不是通过在测试阶段爬行目录结构来重新加载它），我们将数据转储到磁盘：
  ...
  // Build the tree index and save it to disk最后，我们创建KdTree，并将其结构保存到磁盘：
